...
class ListaPresidente
{
    ...
    public void ordenar()
    {
        ...
        try
        {
            File arquivo=new File("618557_radixsort.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
    ...
    public void ordenacaoRadixSort()
    {
        ...
        for(int exp=1;m/exp>0;exp*=10)
            countSort(exp);
    }//fim do metodo ordenacaoRadixSort

    /**
     * Funcao do metodo: Este metodo serve para executar parte do processo de ordenacao do RadixSort.
     * @param exp Digito 
     */
    public void countSort(int exp)
    {
        Presidente output[]=new Presidente[n];
        int i;
        ...
        for(i=0;i<n;i++)
        {
            count[(presidentes[i].getID()/exp)%10]++;
        }
        ...
        for(i=n-1;i>=0;i--)
        {
            output[count[(presidentes[i].getID()/exp)%10]-1]=presidentes[i];
            count[(presidentes[i].getID()/exp)%10]--;
        }

        for(i=0;i<n;i++)
        {
            presidentes[i]=output[i];
        }
    }//fim do metodo countSort

    /**
     * Funcao do metodo: Este metodo serve para obter o maior valor do id dos presidentes.
     * @return Valor inteiro contendo a maior idade.
     */
    public int getMax()
    {
        int mx=presidentes[0].getID();
        for(int i=0;i<n;i++)
        {
            if(presidentes[i].getID()>mx)
                mx=presidentes[i].getID();
        }
        return mx;
    }//fim do metodo getMax    

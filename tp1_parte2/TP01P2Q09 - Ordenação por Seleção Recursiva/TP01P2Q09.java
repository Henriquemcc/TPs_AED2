...
class ListaPresidente
{
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo presidentes tendo como chave o id de cada presidente e utilizando o SelectionShort recursivo.
     */
    public void ordenar()
    {
        long comeco=new Date().getTime();
        ...
        try
        {
            File arquivo=new File("618557_selecao.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo presidentes tendo como chave o id de cada presidente e utilizando o SelectionShort recursivo.
     * @param indice Posicao que sera ordenada e que a partir dela sera chamado o mesmo metodo de forma recursiva com o seu valor +1.
     */
    public void ordenar(int indice)
    {
        if(indice!=n)
        {
            int k=indiceMinimo(indice, n-1);
            if(k!=indice)
                swap(k, indice);
            ordenar(indice+1);
        }

...
class ListaPresidente
{
    ...
    public void ordenar()
    {
        ...
        try
        {
            File arquivo=new File("618557_ordenacao_bolha.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
    ...
    /**
     * Funcao do metodo: Este metodo serva para ordenar o arranjo de objetos do tipo presidente.
     */
    public void ordenacaoBolha()
    {
        for(int i=(n-1);i>0;i--)
        {
            for(int j=0;j<i;j++)
            {
                if(Utilitarios.removerAcentos(presidentes[j].getLocalNascimento()).compareTo(Utilitarios.removerAcentos(presidentes[j+1].getLocalNascimento()))>0)
                    swap(j, j+1);
            }
        }
    }//fim do metodo ordenacaoBolha

    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo que posseum idades iguais, pelo nome.
     */
    public void ordenarIguais()
    {
        for(int i=0;i<n;i++)
        {
            for(int j=i+1;j<n;j++)
            {
                if(Utilitarios.removerAcentos(presidentes[i].getLocalNascimento()).equals(Utilitarios.removerAcentos(presidentes[j].getLocalNascimento())))
                {
                    if(Utilitarios.removerAcentos(presidentes[i].getNome()).compareTo(Utilitarios.removerAcentos(presidentes[j].getNome()))>0)
                    {
                        swap(i, j);
                    }

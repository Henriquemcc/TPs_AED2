...
class ListaPresidente
{
    ...
    /**
     * Funcao do metodo: Este metodo serve para imprimir (na saida padrao) os objetos do tipo Presidente salvos no arranjo presidentes.
     * @param esquerda Limite inferior do arranjo a ser mostrado.
     * @param direita Limite superior do arranjo a ser mostrado.
     */
    public void mostrar(int esquerda, int direita)
    {
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo presidentes tendo como chave o tamanho da pagina de cada presidente e utilizando o HeapSort.
     */
    public void ordenar()
    {
        long comeco=new Date().getTime();
        ordenacaoParcialHeapSort(10);
        long fim=new Date().getTime();
        
        //Escrevendo no arquivo
        try
        {
            File arquivo=new File("618557_ordenacao_parcial_heapsort.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
    ...
    /**
     * Funcao do metodo: Este metodo serva para ordenar parcialmente o arranjo de objetos do tipo presidente.
     * @param k Limite superior do arranjo.
     */
    public void ordenacaoParcialHeapSort(int k)
    {
        //Alterar o vetor ignorando a posicao zero
        Presidente tmp[]=new Presidente[n+1];
        for(int i=0;i<n;i++)
        {
            tmp[i+1]=presidentes[i];
        }
        presidentes=tmp;
        ...
        tmp=presidentes;
        presidentes=new Presidente[n];
        for(int i=0;i<n;i++)
        {
            presidentes[i]=tmp[n-i];
        }

    }//fim do metodo ordenacaoParcialHeapsort

    /**
     * Funcao do metodo: Este metodo serva para construir o Heap, essenciaal para o funcionamento do HeapSort.
     * @param int tamHep Tamanho do heap que sera construido.
     */
    public void constroi(int tamHeap)
    {
        int i=tamHeap;
        while(i>1 && presidentes[i].getDataNascimento().compareTo(presidentes[i/2].getDataNascimento())<0)
        {
            swap(i, i/2);
            i/=2;
        }
    }//fim do metodo constroi

    /**
     * Funcao do metodo: Este metodo serva para reconstruir o Heap, essenciaal para o funcionamento do HeapSort.
     * @param int tamHep Tamanho do heap que sera reconstruido.
     */
    public void reconstroi(int tamHeap)
    {
        ...
        while(i<=(tamHeap/2))
        {
            if(presidentes[2*i].getDataNascimento().compareTo(presidentes[2*i+1].getDataNascimento())<0 || 2*i==tamHeap)
                filho=2*i;
            else
                filho=2*i+1;

            if(presidentes[i].getDataNascimento().compareTo(presidentes[filho].getDataNascimento())>0)
            {
                swap(i, filho);
                i=filho;
            }
...
class Presidente
{
    ...
    public  void calcularIdade()
    {
        ...
        try
        {
            anoNascimento=this.getDataNascimento().getYear();
            anoHoje=LocalDateTime.now().getYear();
        }
        catch(Exception e)
        {
            //System.out.println(e);

...
class ListaPresidente
{
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo presidentes tendo como chave o tamanho da pagina de cada presidente e utilizando o InsertionSort.
     */
    public void ordenar()
    {
        long comeco=new Date().getTime();
        ordenacaoParcialInsercao(10);
        long fim=new Date().getTime();
        
        //Escrevendo no arquivo
        try
        {
            File arquivo=new File("618557_ordenacao_parcial_insercao.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
     ...
     */
    public void ordenacaoParcialInsercao(int k)
    {
       for(int i=1;i<n;i++)
       {
            ...
            while((j>=0) && presidentes[j].getPaginaTam()>tmp.getPaginaTam())
            {
                presidentes[j+1]=presidentes[j];
                j--;
            }
            presidentes[j+1]=tmp;
    ...
    }//fim do metodo ordenacaoParcialInsercao

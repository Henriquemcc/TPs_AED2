...
class ListaPresidente
{
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo do tipo Presidente.
     */
    public void ordenar()
    {
        ordenar(0, n-1);
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo do tipo Presidente.
     * @param inicio Posicao limite inferior do arranjo.
     * @param fim Posicao limite superior do arranjo.
     */
    public void ordenar(int inicio, int fim)
    {
        ...
        if(fim-inicio>=1)
        {
            String pivo=Utilitarios.removerAcentos(presidentes[inicio].getNome());
            while(k>i)
            {
                while(Utilitarios.removerAcentos(presidentes[i].getNome()).compareTo(pivo)<=0 && i<=fim && k>i)
                    i++;
                while(Utilitarios.removerAcentos(presidentes[k].getNome()).compareTo(pivo)>0 && k>=inicio && k>=i)
                    k--;
                if(k>i)
                    swap(i, k);
            }

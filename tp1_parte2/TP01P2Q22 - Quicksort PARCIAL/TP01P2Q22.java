...
class ListaPresidente
{
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo presidentes tendo como chave o inicio do mandato de cada presidente e utilizando o QuickSort.
     */
    public void ordenar()
    {
        long comeco=new Date().getTime();
        ordenacaoParcialQuickSort(10);
        long fim=new Date().getTime();
        
        //Escrevendo no arquivo
        try
        {
            File arquivo=new File("618557_ordenacao_parcial_quicksort.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
     ...
     */
    public void ordenacaoParcialQuickSort(int k)
    {
        ordenacaoParcialQuickSort(0, n-1, k);
    }//fim do metodo ordenacaoParcialQuicksort

    /**
     * Funcao do metodo: Este metodo serve para ordenar os objetos do tipo Presidente do arranjo utilizando como chave a data de inicio do mandato.
     * @param int inicio Inicio do arranjo a ser ordenado.
     * @param int fim Fim do arranjo a ser ordenado.
     */
    public void ordenacaoParcialQuickSort(int inicio, int fim, int limite)
    {
        int i=inicio;
        int k=fim;
        if(fim-inicio>=1)
        {
            LocalDateTime pivo=presidentes[inicio].getInicioMandato();
            while(k>i)
            {
                while(presidentes[i].getInicioMandato().compareTo(pivo)<=0 && i<=fim && k>i)
                    i++;

                while(presidentes[k].getInicioMandato().compareTo(pivo)>0 && k>=inicio && k>=i)
                    k--;
            ...
            swap(inicio, k);
            ordenacaoParcialQuickSort(inicio, k-1, limite);
            if(k<limite)
                ordenacaoParcialQuickSort(k+1, fim, limite);
        }
    }//fim do metodo ordenacaoParcialQuickSort

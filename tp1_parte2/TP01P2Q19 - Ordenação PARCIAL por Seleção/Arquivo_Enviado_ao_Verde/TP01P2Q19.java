...
class ListaPresidente
{
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo presidentes tendo como chave o id de cada presidente e utilizando o SelectionSort.
     */
    public void ordenar()
    {
        long comeco=new Date().getTime();
        ordenacaoParcialSelecao(10);
        long fim=new Date().getTime();
        
        //Escrevendo no arquivo
        try
        {
            File arquivo=new File("618557_ordenacao_parcial_selecao.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
     ...
     */
    public void ordenacaoParcialSelecao(int k)
    {
        for(int i=0;i<k;i++)
        {
            int menor=i;
            for(int j=(i+1);j<n;j++)
            {
                if(presidentes[menor].getID()>presidentes[j].getID())
                    menor=j;
            }
            swap(menor, i);
        }
    }//fim do metodo ordenacaoParcialSelecao

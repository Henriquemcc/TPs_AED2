...
class ListaPresidente
{
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo presidentes tendo como chave idade de cada presidente e utilizando o CountingSort.
     */
    public void ordenar()
    {
        long comeco=new Date().getTime();
        ...
        try
        {
            File arquivo=new File("618557_countingsort.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
    ...
    public void ordenarIguais()
    {
        for(int i=0;i<n;i++)
        {
            for(int j=i+1;j<n;j++)
            {
                if(presidentes[i].getIdade()==presidentes[j].getIdade())
                {
                    if(presidentes[i].getNome().compareTo(presidentes[j].getNome())>0)
                    {
                        swap(i, j);
                    }
    ...
    /**
	 * Funcao do metodo: Algoritmo de ordenacao Countingsort.
	 */
    public void countingSort()
    {
        //Array para contar o numero de ocorrencias de cada elemento
        int count[]=new int[getMaior()+1];
        Presidente ordenado[]=new Presidente[n];
        ...
        for(int i=n-1;i>=0;i--)
        {
            ordenado[count[presidentes[i].getIdade()]-1]=presidentes[i];
            count[presidentes[i].getIdade()]--;
        }

        //Copiando para o array original
        for(int i=0;i<n;i++)
            presidentes[i]=ordenado[i];
    }//fim do metodo countingSort

    /**
	 * Funcao do metodo: Retorna o maior elemento do array.
    * @return maior elemento
	 */
    public int getMaior()
    {
        int maior=presidentes[0].getIdade();
        for(int i=0;i<n;i++)
        {
            if(maior<presidentes[i].getIdade())
                maior=presidentes[i].getIdade();
        }
        return maior;
    }//fim do metodo getMaior
...
class Utilitarios
{
     ...
     * @param s String que todos os acentos ser�o removidos

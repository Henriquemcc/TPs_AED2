...
class ListaPresidente
{
    ...
    /**
     * Funcao do metodo: Este metodo serve para ordenar todos os elementos do arranjo presidentes tendo como chave o inicio do mandato de cada presidente e utilizando o MergeSort.
     */
    public void ordenar()
    {
        long comeco=new Date().getTime();
        ...
        try
        {
            File arquivo=new File("618557_mergesort.txt");
            if (!arquivo.exists())
                arquivo.createNewFile();
    ...
    /**
     * Funcao do metodo: Este metodo serve para executar a ordenacao usando o algoritmo MergeSort
     */
    ...
    /**
     * Funcao do metodo: Este metodo serve para juntar todos os elementos durante o processor de ordenacao do mergesort.
     * @param esquerda Posicao Limite inferior do arranjo.
     * @param direita Posicao Limite superior do arranjo.
     */
    public void merge(int esquerda, int meio, int direita)
    {
        ...
        Presidente ESQUERDA[]=new Presidente[n1];
        Presidente DIREITA[]=new Presidente[n2];
        
        for(int i=0;i<n1;i++)
        {
            ESQUERDA[i]=presidentes[esquerda+i];
        }
        for(int j=0;j<n2;j++)
        {
            DIREITA[j]=presidentes[meio+1+j];
        }
        ...
        while(i<n1 && j<n2)
        {
            if(ESQUERDA[i].getFimMandato().compareTo(DIREITA[j].getFimMandato())<=0)
            {
                presidentes[k]=ESQUERDA[i];
                i++;
            }
            else
            {
                presidentes[k]=DIREITA[j];
                j++;
            }
            k++;
        }
    ...
    {
        sort(0, n-1);
    }//fim do metodo ordenacaoMergeSort
